...
void VinylControlManager::updateSignalQualityListeners() {
    ...
    // The processor can enqueue many reports between UI ticks -- up to the FIFO
    // capacity (256) -- but only the most recent report per vinyl input is visible
    // at the ~15 Hz UI update rate. Drain the FIFO keeping only the latest report
    // per processor, then emit each once, instead of forwarding every (mostly stale)
    // report to every listener. Each report carries a large scope[] buffer, so the
    // redundant emits were a real CPU waste. Fixes issue #16536.
    VinylSignalQualityReport latest[kMaximumVinylControlInputs];
    bool haveReport[kMaximumVinylControlInputs] = {false};
    VinylSignalQualityReport report;
    while (signalQualityFifo->read(&report, 1) == 1) {
        if (report.processor < kMaximumVinylControlInputs) {
            latest[report.processor] = report;
            haveReport[report.processor] = true;
        }
    }
    for (int i = 0; i < kMaximumVinylControlInputs; ++i) {
        if (!haveReport[i]) {
        ...
        foreach (VinylSignalQualityListener* pListener, m_listeners) {
            pListener->onVinylSignalQualityUpdate(latest[i]);
        }
    }
}
