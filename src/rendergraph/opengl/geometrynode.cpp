...
void GeometryNode::markDirtyGeometry() {
    // Tell the backend node the vertex data changed so render() re-uploads it
    // to the persistent VBO. GeometryNode is a NodeInterface<BaseGeometryNode>,
    // so it inherits markGeometryDirty() directly.
    markGeometryDirty();
}
