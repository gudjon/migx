...
void TrackDAO::exportToSidecar(const Track& track) const {
    const QString location = track.getLocation();
    if (location.isEmpty()) {
        return;
    }
    mixxx::FileInfo fileInfo(location);
    QString sidecarDirPath = fileInfo.location() + ".migx";
    if (!QDir().mkpath(sidecarDirPath)) {
        kLogger.warning()
                << "Could not create FSL sidecar directory"
                << sidecarDirPath;
        return;
    }

    QJsonObject trackObj;
    trackObj["bpm"] = track.getBpm();
    ...
    const QString sidecarFilePath =
            QDir(sidecarDirPath).filePath(QStringLiteral("track.json"));

    QFile existingFile(sidecarFilePath);
    if (existingFile.exists()) {
        if (existingFile.open(QIODevice::ReadOnly)) {
            if (existingFile.readAll() == sidecarJson) {
                return;
            }
            existingFile.close();
        } else {
            kLogger.warning()
                    << "Could not read existing FSL sidecar"
                    << sidecarFilePath
                    << existingFile.errorString();
        }
    }

    QSaveFile sidecarFile(sidecarFilePath);
    if (!sidecarFile.open(QIODevice::WriteOnly)) {
        kLogger.warning()
                << "Could not open FSL sidecar for writing"
                << sidecarFilePath
                << sidecarFile.errorString();
        return;
    }
    const qint64 sidecarJsonSize = static_cast<qint64>(sidecarJson.size());
    const qint64 bytesWritten = sidecarFile.write(sidecarJson);
    if (bytesWritten != sidecarJsonSize) {
        kLogger.warning()
                << "Could not write complete FSL sidecar"
                << sidecarFilePath
                << bytesWritten
                << "of"
                << sidecarJsonSize
